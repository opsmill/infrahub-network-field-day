...
This command is fired as a `before_specify` hook by the `infrahub-speckit` extension. It runs every time the active integration invokes the core specify workflow, for example `/speckit-specify` in Codex skills or `/speckit.specify` in Claude Code slash commands. If `.infrahub.yml` is not present in the repository root, this command is a no-op (exit cleanly without emitting any directives).
...
After installing, restart this session and re-run the specify command.
```
...
- **If the command fails or Infrahub is not reachable**, stop and warn the user:
  ```
  Infrahub is not reachable. Start or configure an Infrahub instance first,
  including INFRAHUB_ADDRESS and authentication if your project uses a remote
  lab. Then re-run the specify command.
  ```
...
The user's original input to the specify command is available as `$ARGUMENTS` (the same string passed to the core skill). Match it case-insensitively against this table. A prompt can match multiple rows.
...
| **Objects**   | objects, object data, seed data, populate, import, instance data | `spec-objects-template` |
...
After completing this cycle, run the specify command again for the next artifact.
```

Dependency order (earlier must complete first):

1. Schema (always first — everything depends on the data model)
2. Objects, Check, Generator, Transform, Menu (all depend on schema; independent of each other)
...
| **Objects**   | `infrahub-managing-objects`    | Object YAML structure, relationship values, hierarchy, range expansion, load order |
...
1. Prefer `.specify/extensions/infrahub-speckit/templates/<template-name>.md` (e.g. `spec-schema-template.md`). These templates ship with this extension.
2. If that file does not exist, fall back to `.specify/templates/spec-template.md`. Emit a warning that the Infrahub-specific spec template is missing.
...
```
[infrahub-speckit routing — applies to this specify run only]
...
If multiple artifact types were detected in Step 5, also remind the user at the end of the spec cycle to re-run the specify command for the next artifact in the chain. (This reminder lives in the hook output; the core skill will surface it when reporting completion.)