...
`tooling/` is a second Kubernetes cluster — `tool-node1`, hanging off the firewall's `ge-0/0/6`
on a bridge the **host** is also on — carrying two things: Dex, the lab's identity provider, and
Backstage, the portal people request services through. `scripts/deploy_tooling.sh` deploys it and
`uv run invoke tooling` is the front door.

**Infrahub does not deliver any of it, and cannot.** Dex is what Infrahub authenticates users
against and Backstage is where requests come from, so anything waiting on an Infrahub merge would
have to exist before it could be merged. The bridge is what breaks that: being L2-adjacent to the
host, this cluster comes up with no fabric and no firewall in the path, while branch users still
reach it the hard way, through WAN → border-leaf1 → fw1 → zone `tooling`.
...
- **There is exactly one Dex, and it is reachable only from the branch side.** An OIDC issuer is
  ...
  **an operator signs in with the local `admin` account**, which sits on the same login page. A
  second Dex on the host was built for them and removed — two issuers is two identities for one
  person, and the same `alice` would be a different Infrahub account depending on which network
  she came in on. Before moving the issuer, test from every browser that has to reach it; an
  earlier consolidation was tested everywhere except a remote operator's machine.
- **The portal is served over HTTPS with a self-signed certificate, and that is not about
  privacy.** The frontend calls `crypto.randomUUID`, which the browser populates only in a
  secure context, so over plain HTTP a user signs in and the app dies with
  `globalThis.crypto.randomUUID is not a function`. `localhost` is a secure context by exception,
  which is why this is invisible from the host.
- **The certificate is generated by the deploy script, not by Backstage.** `backend.https: true`
  makes Backstage generate its own inside the container, so a pod restart produces a new one and
  the branch user re-accepts a browser exception every time. The script's lives in a Secret.
