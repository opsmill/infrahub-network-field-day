...
**One rule, learned the expensive way and then found to be broader than first
written.** A selection set on an ABSTRACT type -- an interface or a union --
must select `__typename`.

The first version of this test checked only for inline fragments, because that
was the shape of the first two defects found. It then missed a third:
`generate_server_placement.gql` selected `template { node { id ... } }` where
the peer is the generic `CoreObjectTemplate`, with no fragment anywhere. The
generated model was a discriminated union all the same, and every request failed
to parse. An inline fragment is a *symptom* of an abstract peer, not the cause.
...
SCHEMA_PATH = REPO_ROOT / "schema.graphql"


@lru_cache(maxsize=1)
def _schema() -> graphql.GraphQLSchema:
    """The exported Infrahub schema, which is what makes the typed walk possible.

    Cached: this schema is large and building it per test made the suite
    noticeably slower than everything else in it combined.

    Regenerated by `infrahubctl graphql export-schema`. If it is stale the walk
    below reports fields it cannot resolve rather than silently skipping them.
    """
    return graphql.build_schema(SCHEMA_PATH.read_text(encoding="utf-8"))


def _abstract_offences(document: graphql.DocumentNode, schema: graphql.GraphQLSchema) -> list[str]:
    """Every selection set on an abstract type that does not select `__typename`.

    Walks with `TypeInfo` so the *type* of each field is known, rather than
    inferring from the presence of a fragment. That is the difference between
    catching two of the three defects found so far and catching all of them.
    """
    offences: list[str] = []
    type_info = graphql.TypeInfo(schema)

    class _Visitor(graphql.Visitor):
        def enter_selection_set(self, node: graphql.SelectionSetNode, *_args: object) -> None:
    ...
    graphql.visit(document, graphql.TypeInfoVisitor(type_info, _Visitor()))
...
@pytest.mark.parametrize("path", _queries(), ids=lambda p: str(p.relative_to(REPO_ROOT)))
def test_every_abstract_selection_set_has_a_discriminator(path: Path) -> None:
    """THE GENERAL RULE, which the fragment check above is only a symptom of.

    A peer that is an interface or a union produces a discriminated union in the
    generated model whether or not the query uses a fragment. Found the hard way:
    `generate_server_placement.gql` selected a `CoreObjectTemplate` with no
    fragment at all, and every request failed to parse.
    """
    if str(path.relative_to(REPO_ROOT)) in HAND_WRITTEN_MODELS:
        pytest.skip("model is hand-written; see HAND_WRITTEN_MODELS")
    offences = _abstract_offences(graphql.parse(path.read_text(encoding="utf-8")), _schema())
    assert not offences, (
        f"{path.relative_to(REPO_ROOT)} selects from these abstract types without __typename: "
        + ", ".join(offences)
        + "\n\nThe generated model makes each one a discriminated union keyed on __typename."
    )


def test_the_exported_schema_is_present_and_parses() -> None:
    """Without it the typed rule above silently has nothing to check."""
    assert SCHEMA_PATH.is_file(), "schema.graphql is missing; run `infrahubctl graphql export-schema`"
    assert _schema().query_type is not None
