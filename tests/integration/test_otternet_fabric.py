"""End-to-end test that Infrahub reproduces the deployed OTTERNET lab exactly.
...
generator chain against the ``OTTERNET_FABRIC`` design. It then renders the AVD EOS
...
(``tests/integration/golden/otternet``).
...
PIPELINE_BRANCH = "otternet-pipeline"
FABRIC_NAME = "OTTERNET_FABRIC"
POD_NAME = "otternet-pod1"

GOLDEN_DIR = Path(__file__).parent / "golden" / "otternet"
...
# The uplink each leaf takes on each spine, from lab/otternet.clab.yml. Pinned here
...
@pytest.mark.e2e
class TestOtternetFabric(TestInfrahubDockerClient):
    """Design-to-configuration parity with the running OTTERNET lab."""
    ...
    async def test_fabric_generator_cascade(self, client: InfrahubClient, infrahub_port: int) -> None:
        ...
        await wait_until(
            ...
            describe="the seven OTTERNET switches after the generator cascade",
        )
...
async def _device_identity_report(client: InfrahubClient, branch: str) -> dict[str, dict[str, int | None]]:
    """Hostname -> node ID and ASN for every device in the OTTERNET pod."""
    pods = await client.filters(kind="NetworkPod", name__value=POD_NAME, branch=branch)
    if not pods:
        return {}
