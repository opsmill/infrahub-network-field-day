#!/usr/bin/env python3
"""Regenerate the NFD41 golden EOS configurations from the lab's AVD inputs.

The golden set in ``tests/integration/golden/nfd41`` is the reference both parity
tests compare against, so where it comes from matters. It is *not* Infrahub
output — that would make the tests circular. It is rendered from the NFD41 lab's
...
Usage:
    uv run python scripts/regenerate_nfd41_golden.py            # check only
    uv run python scripts/regenerate_nfd41_golden.py --write    # update the golden set

Point --lab at the NFD41 lab checkout if it is not beside this repository.
"""
...
GOLDEN_DIR = REPO_ROOT / "tests" / "integration" / "golden" / "nfd41"

POD_NAME = "nfd41-pod1"

# Lab hostname -> the name Infrahub's generators produce. Rack indices are
# K8S_LEAFS=1, APP_LEAFS=2, BORDER_LEAFS=3 (objects/25_nfd41_racks.yml).
RENAME: dict[str, str] = {
...
NODE_TYPE_FILE = {
    "spine1": "NFD41_SPINES.yml",
    "spine2": "NFD41_SPINES.yml",
    "k8s-leaf1": "NFD41_L3LEAFS.yml",
    "k8s-leaf2": "NFD41_L3LEAFS.yml",
    "app-leaf1": "NFD41_L3LEAFS.yml",
    "app-leaf2": "NFD41_L3LEAFS.yml",
    "border-leaf1": "NFD41_L3LEAFS.yml",
}
...
def build_hostvars(group_vars: Path) -> dict[str, dict[str, Any]]:
    fabric: dict[str, Any] = {}
    for path in sorted((group_vars / "NFD41_FABRIC").glob("*.yml")):
...
def main() -> int:
    ...
    parser.add_argument("--lab", type=Path, default=DEFAULT_LAB, help="path to the NFD41 lab checkout")
    ...
    if not group_vars.is_dir() or not deployed.is_dir():
        print(f"error: no NFD41 lab AVD data under {args.lab}", file=sys.stderr)
        print("       pass --lab <path-to-nfd41-lab>", file=sys.stderr)
        return 1
